...
static size_t MAX_PRIORITY = 1UL << 30;
...
struct euler_tour_tree_exception : public std::runtime_error {
    euler_tour_tree_exception(const std::string& __arg)
            : runtime_error(__arg) {}
};
...
struct euler_tour_tree {
    ...
    euler_tour_tree(size_t cnt_vertices)
    ...
    bool is_connected(size_t u, size_t v) const {
        check(u);
        check(v);
        if (u == v) {
            return true;
...
struct dc_exception : public std::runtime_error {
    dc_exception(const std::string& __arg)
            : runtime_error(__arg) {}
};

struct aux_edge_handler {
    aux_edge_handler(euler_tour_tree& ett) : ett_(ett) {};
...
struct dc {
    explicit dc(size_t cnt_vertices) : cnt_vertices_(cnt_vertices) {
        init_level(0);
    }
    ...
    bool is_connected(size_t u, size_t v) const {
        check(u);
        check(v);
        return etts_[0]->is_connected(u, v);
    }
...
private:
    ...
    void up_span(treap_node* v, size_t level) {
        ...
        if (v->u < v->v) etts_[level + 1]->link(v->u, v->v);
    }
